      ...
      <div className="max-w-3xl mx-auto pt-8 px-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            <div className="h-12 w-12 rounded-full bg-green-700 text-white flex items-center justify-center text-sm font-bold">
              {initials}
            </div>
            <div>
              <h1 className="text-2xl font-bold">Help & Support</h1>
              <p className="text-sm text-gray-500">{user?.full_name || 'User'} · {user?.role || ''}</p>
            </div>
          </div>
          <button
            onClick={loadTickets}
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium ${
              activeTab === 'list' ? 'bg-green-700 text-white' : 'bg-white border border-gray-200 text-gray-700'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium ${
              activeTab === 'new' ? 'bg-green-700 text-white' : 'bg-white border border-gray-200 text-gray-700'
            }`}
          >
        ...
        {activeTab === 'list' ? (
          loading && tickets.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm p-10 text-center text-gray-400">Loading tickets...</div>
          ) : tickets.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm p-10 text-center">
              <LifeBuoy className="h-12 w-12 text-gray-300 mx-auto mb-3" />
              <p className="text-gray-500">No tickets yet. Need help? Raise one in 30 seconds.</p>
              <button
                onClick={() => setActiveTab('new')}
            ...
            <div className="space-y-3">
              {tickets.map((t) => (
                <div key={t.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  ...
                  <p className="text-gray-700 text-sm mb-2">{t.description}</p>
                  {t.order && (
                    <p className="text-xs text-gray-500 mb-2">
                      Order #{t.order.id.slice(0, 8)} · ₹{t.order.total_amount}
                    </p>
                  )}
                  <div className="flex flex-wrap gap-4 text-xs text-gray-400 border-t border-gray-100 pt-2 mt-2">
                  ...
                  {t.resolution_note && (
                    <div className="mt-2 bg-green-50 border border-green-200 text-green-900 rounded-lg px-3 py-2 text-sm flex gap-2">
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="mb-4">
              <label className="block text-sm text-gray-600 mb-1">Describe the issue</label>
              <textarea
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full sm:w-auto px-6 py-2.5 bg-green-700 text-white rounded-lg text-sm hover:bg-green-800 disabled:opacity-60"
            >
